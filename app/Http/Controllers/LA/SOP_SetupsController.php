...
class SOP_SetupsController extends Controller
{
        ...
        if(Module::hasAccess("SOP_Setups", "create")) {
            ...
            for($i = 1; $i<=$reportTo_count; $i++)
            {
                if($request->input('reportTo_'.$i) != null)
                {
                    ...
                    $insert = DB::table('sop_reportto_users')->insertGetId([
                        "created_at" => $today,
    ...
    public function getSOPDataByPIC(Request $request){
        ...
        $values = DB::table('sop_user_by_id')
            ->select('id', 'work_description as WorkDescription', 'JobType', 'TimeFrame', 'Supportings', 'ReportTo', 'Remark')
    ...
    public function getSOPList(){
        if(Module::hasAccess("SOP_Setups", "view")) {
			if(Entrust::hasRole('SUPER_ADMIN')) {
				$values = DB::table('sop_user_by_id')
                    ->select('id', 'work_description as WorkDescription', 'PIC', 'JobType', 'TimeFrame', 'Supportings', 'ReportTo', 'Remark')
                    ->get();
			} else {
                $pic_id = Auth::user()->id;
				$values = DB::table('sop_user_by_id')
            ...
            $pic_users = DB::table('sop_user_by_id')
                ->select('pic_user_id', 'PIC')
                ->whereNotNull('pic_user_id')
                ->groupBy('sop_user_by_id.pic_user_id')->get();
            return view('la.sop_setups.show', [
                'no_header' => true,
                'no_padding' => "no-padding"
            ])->with('sops', $values)->with('pic_users', $pic_users);
		}else {
            return redirect(config('laraadmin.adminRoute') . "/");
        }
    }
